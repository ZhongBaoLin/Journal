  ...
  let academicYearsCache = []; // масив рядків, напр. ["2025/2026", "2024/2025", ...]
...
async function renderJournal() {
    ...
    for (const student of students) {
        ...
        // Підсумкова оцінка редагується однаково і для заліку, і для екзамену —
        // відрізняється лише назва колонки (див. finalColHeader вище)
...
// При зміні предмета в формі заняття - одразу оновлюємо список розділів
// (включно з пунктом "+ Створити новий розділ", навіть якщо розділів ще немає)
...
window.selectSection = function(value, label, event) {
  ...
  async function loadMyLessons() {
    ...
    snap.forEach(docSnap => {
      ...
      // Показуємо лише заняття з предметів поточного навчального року
  ...
  // ========== ФУНКЦІЇ ДЛЯ ГРУП ==========
  // Показуємо лише групи поточного обраного навчального року
  async function loadGroups() {
    ...
    snap.forEach(docSnap => {
      ...
      if (currentAcademicYear && year !== currentAcademicYear) return; // фільтр за поточним навчальним роком
      groups.push({ id: docSnap.id, name: g.name, year, studentCount: studentCountByGroup[docSnap.id] || 0 });
    });
    if (groups.length === 0) {
      container.innerHTML = `<div class="empty-state">Груп не знайдено${currentAcademicYear ? ` за ${currentAcademicYear} н.р.` : ''}</div>`;
      populateSelects();
      return;
    }
...
async function addGroupAndUpdateSelects(groupName, academicYear) {
  ...
  async function loadStudents() {
    ...
    groupsSnap.forEach(d => {
      ...
      // Показуємо у фільтрі лише групи поточного навчального року
    ...
    // Обмежуємо студентів лише групами поточного навчального року
  ...
  // ========== СТАТИСТИКА СТУДЕНТА (МОДАЛЬНЕ ВІКНО) ==========
  ...
  // Отримуємо навчальний рік групи (студент завжди успадковує рік від своєї групи)
  ...
  // Показуємо лише предмети/дисципліни поточного обраного навчального року
...
window.showSubjectInfo = async (subjectId) => {
  ...
  async function updateStats() {
    ...
    // Групи лише поточного навчального року
    ...
    // Студенти лише з груп поточного навчального року
    let studentsCount = 0;
    studentsSnap.forEach(s => { if (currentYearGroupIds.has(s.data().groupId)) studentsCount++; });

    // Предмети/дисципліни лише поточного навчального року
    let subjectsCount = 0;
  ...
  // Навчальний рік для журналу більше не обирається окремо — використовується
  // спільний currentAcademicYear (обраний через кнопку "Навчальний рік" вгорі).
  // Тут залишається лише вибір групи та предмету/дисципліни в межах цього року.
...
function populateSelects() {
    ...
    // Завантажуємо предмети для фільтрів — лише за поточний навчальний рік
  ...
  // Firestore не дозволяє "/" у ID документа (це сприймається як розділювач шляху),
  // тому для ID документа використовуємо санітизовану версію, а сам рядок року
  // ("2025/2026") зберігаємо як звичайне поле "year" для показу користувачу.
  ...
  function populateCareerYearSelects() {
    ...
    // Селект для року початку нового навчального року (в модалці вибору року)
  ...
  // Профіль вважається завершеним лише якщо всі обов'язкові поля заповнені —
  // це страхує і від старих/пошкоджених записів, де profileCompleted міг бути
  // виставлений некоректно.
  ...
  function openRegistrationModal() {
    wizardStep = 1;
    // Якщо в користувача вже є частина даних (повторний вхід із незаповненим профілем) —
    // підставляємо те, що вже збережено, і показуємо інший заголовок.
  ...
  function applyWizardStep() {
    ...
    const titles = wizardIsFirstTime
      ? { 1: 'Особисті дані', 2: 'Педагогічний досвід', 3: 'Підтвердження' }
      : { 1: 'Особисті дані', 2: 'Педагогічний досвід', 3: 'Підтвердження' };
  ...
  async function finishRegistration() {
    ...
    try {
      ...
      // Далі — обов'язковий вибір навчального року
  ...
  window.closeYearModal = function () {
    if (yearModalMandatory) return; // не можна закрити, поки рік не обрано вперше
  ...
  window.confirmYearSelection = async function () {
    ...
    try { await updateDoc(doc(db, "users", currentTeacherId), { currentAcademicYear: val }); } catch (e) { /* не критично */ }
    ...
    if (wasMandatory) {
      ...
      // Рік змінено вручну пізніше — очищуємо збережений вибір групи/предмета
      // (вони стосувалися попереднього року) та перезавантажуємо ВСІ дані,
      // прив'язані до навчального року: групи, студенти, предмети, заняття та журнал.
      ...
      // Перезавантажуємо всі колекції даних (групи, предмети, студенти, заняття) та статистику.
      // loadGroups()/loadMySubjects() всередині loadAllData() самі викликають populateSelects(),
      // який оновлює форми заняття/студента/предмета та фільтри журналу (populateJournalFilters).
      await loadAllData();
  ...
  async function continueAfterAuth() {
    ...
    // (populateJournalFilters сам відновлює останню обрану групу/предмет у межах поточного року)
    if (document.getElementById("tab-journal").classList.contains("active")) {
      await populateJournalFilters();
    }
  ...
  window.openAddLessonForm = async function() {
    ...
    // Наповнюємо дропдаун розділів для вже обраного предмета (щоб одразу
    // побачити пункт "+ Створити новий розділ", навіть якщо розділів ще немає)
    const subjectId = document.getElementById("lessonSubject").value;
    await updateSectionSelect(subjectId);
  };
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      if (snap.exists() && snap.data().role === "teacher") {
        ...
        // Завантажуємо спільний список навчальних років і заповнюємо селекти стажу
        await loadAcademicYears();
        populateCareerYearSelects();

        // Крок 1: якщо профіль ще не заповнений — показуємо коротку реєстрацію.
        // Подальший запуск (continueAfterAuth) відбудеться після реєстрації + вибору року.
        if (!isProfileComplete(currentUserData)) {
          openRegistrationModal();
          return;
        }

        // Крок 2: перевіряємо, чи обрано навчальний рік для поточної сесії/пристрою

...
function populateSelects() {
  ...
  async function ensureAcademicYearsSeed() {
    ...
    const seedYears = ["2022/2023", "2023/2024", "2024/2025", "2025/2026"];
    for (const y of seedYears) {
      const startYear = parseInt(y.split('/')[0]);
      await setDoc(doc(db, "academicYears", y), { year: y, startYear, createdAt: new Date().toISOString() });
    }
  }
  ...
  async function loadAcademicYears() {
    ...
    const snap = await getDocs(collection(db, "academicYears"));
    const years = [];
    snap.forEach(d => years.push(d.data().year || d.id));
  ...
  function openRegistrationModal() {
    ...
    document.getElementById("regStartYear").value = "";
  ...
  window.addNewAcademicYear = async function () {
    ...
    if (!academicYearsCache.includes(yearStr)) {
      await setDoc(doc(db, "academicYears", yearStr), { year: yearStr, startYear: start, createdAt: new Date().toISOString(), createdBy: currentTeacherId });

...
function populateSelects() {
  ...
  window.confirmYearSelection = async function () {
    ...
    if (wasMandatory) {
      ...
      // Рік змінено вручну пізніше — за наявності, підставляємо його у фільтр журналу
      const journalYearSel = document.getElementById("journalYearSelect");
      if (journalYearSel && journalYearSel.querySelector(`option[value="${val}"]`)) {
        journalYearSel.value = val;
        journalYearSel.dispatchEvent(new Event('change'));
      }
    }
  };
  ...
  async function continueAfterAuth() {
    ...
    if (document.getElementById("tab-journal").classList.contains("active")) {
      await populateJournalFilters();
      const lastYear = sessionStorage.getItem(STORAGE_KEYS.lastYear) || currentAcademicYear;
      const lastGroup = sessionStorage.getItem(STORAGE_KEYS.lastGroup);
      const lastSubject = sessionStorage.getItem(STORAGE_KEYS.lastSubject);

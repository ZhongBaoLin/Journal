...
async function saveAttendance(lessonId, studentId, value) {
  ...
  else if (!isNumericGrade(existing.docs[0].data().grade)) await updateDoc(doc(db, "grades", existing.docs[0].id), data);
}
...
async function renderAttendance() {
  ...
  for (const st of students) {
    ...
    for (const lesson of lessons) {
      ...
      else if (isNumericGrade(g)) { cells += `<td class="att-cell att-cell-locked" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="" title="Є оцінка (${g}) — студент був присутній">✓</td>`; }
...
document.getElementById("attendanceTableContainer").addEventListener('click', (e) => {
  ...
  if (!cell || cell.classList.contains('att-cell-locked')) return;
  const order = ['', 'н', 'св'];

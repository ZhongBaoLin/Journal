...
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc, updateDoc, setDoc, query, where, getDoc } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
async function updateStudentStats(studentId) {
  ...
  if (row) {
      ...
      if (nCell) { 
          nCell.className = nCount > 0 ? 'cell-h-active' : 'stat-cell-red-text'; 
          nCell.textContent = nCount; 
      }
      
      const svCell = row.querySelector('td:nth-last-child(2)');
      if (svCell) { 
          svCell.className = svCount > 0 ? 'cell-sv-active' : 'stat-cell-red-text'; 
...
async function saveAttendance(lessonId, studentId, value) {
  ...
  if (value === '') {
    if (!existing.empty) {
      const g = existing.docs[0].data().grade;
      if (g === 'н' || g === 'св') await deleteDoc(doc(db, "grades", existing.docs[0].id));
    }
    return;
  }
  const data = { lessonId, studentId, grade: value, updatedAt: new Date().toISOString() };
  if (existing.empty) await addDoc(collection(db, "grades"), data);
  else await updateDoc(doc(db, "grades", existing.docs[0].id), data);
}
...
async function renderAttendance() {
  ...
  for (const lesson of lessons) {
    ...
    gs.forEach(g => {
      ...
      allGrades[lesson.id][d.studentId] = d.grade;
    });
  }
  ...
  for (const st of students) {
    let n = 0, sv = 0, cells = '';
    for (const lesson of lessons) {
      const g = allGrades[lesson.id]?.[st.id] || '';
      if (g === 'н') { n++; cells += `<td class="att-cell grade-n" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="н">н</td>`; }
      else if (g === 'св') { sv++; cells += `<td class="att-cell grade-sv" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="св">св</td>`; }
      else if (isNumericGrade(g)) { cells += `<td class="att-cell" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="" data-grade="${g}">—</td>`; }
      else { cells += `<td class="att-cell" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="">—</td>`; }
    }
...
document.getElementById("attendanceTableContainer")?.addEventListener('click', (e) => {
  ...
  // Якщо в клітинці вже є оцінка — попереджаємо, що вона буде замінена
  if (cell.dataset.grade) {
    if (!confirm(`У студента вже є оцінка ${cell.dataset.grade} за це заняття. Замінити її на «${next}»?`)) return;
    delete cell.dataset.grade;
  }
  cell.dataset.value = next;
  cell.className = 'att-cell' + (next === 'н' ? ' grade-n' : next === 'св' ? ' grade-sv' : '');
...
async function renderJournal() {
  ...
  html += `<th class="stat-header-red">Н</th><th class="stat-header-red">СВ</th><th class="stat-cell">${finalColHeader}</th></tr></thead><tbody>`;
  ...
  for (const student of students) {
      ...
      html += `<td class="${nClass}">${stats.n}</td>
               <td class="${svClass}">${stats.sv}</td>`;

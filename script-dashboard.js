...
async function addGroupAndUpdateSelects(groupName, academicYear) {
  ...
  async function loadStudents() {
    ...
    groupsArray.sort((a, b) => { const yearCompare = b.academicYear.localeCompare(a.academicYear); if (yearCompare !== 0) return yearCompare; return a.name.localeCompare(b.name); });

    const groupFilter = document.getElementById("studentGroupFilter");
    const selectedGroup = groupFilter.value;
    let groupOptions = '<option value="all">Всі групи</option>';
    groupsArray.forEach(g => { groupOptions += `<option value="${g.id}">${g.name} (${g.academicYear})</option>`; });

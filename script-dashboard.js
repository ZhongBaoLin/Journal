...
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc, updateDoc, setDoc, query, where, getDoc, deleteField } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
// Ключі для збереження обраного курсу у фільтрах (груп та предметів)
const COURSE_FILTER_KEYS = {
  groups: 'groups_course_filter',
  subjects: 'subjects_course_filter'
};

let currentGroupsCourse = localStorage.getItem(COURSE_FILTER_KEYS.groups) || '1';
let currentSubjectsCourse = localStorage.getItem(COURSE_FILTER_KEYS.subjects) || '1';

// ========== НЕГАЙНЕ ВІДНОВЛЕННЯ ФІЛЬТРІВ ЗА КУРСОМ ДО ЗАВАНТАЖЕННЯ ДАНИХ ==========
(function immediateCourseFilterRestore() {
  const syncCourseButtons = (containerId, course) => {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.querySelectorAll('.subtab-btn').forEach(b => b.classList.toggle('active', b.dataset.course === course));
  };
  syncCourseButtons('groupsCourseTabs', currentGroupsCourse);
  syncCourseButtons('subjectsCourseTabs', currentSubjectsCourse);
})();

// Профіль викладача, спільні навчальні роки та стан майстра реєстрації
let currentUserData = null;
let academicYearsCache = []; 
let currentAcademicYear = null;
let wizardStep = 1;
let yearModalMandatory = false;
let pendingAvatarFile = null;

const ACADEMIC_YEAR_STORAGE_KEY = 'currentAcademicYear';

function getDefaultAcademicYear() {
  return currentAcademicYear || academicYearsCache[0] || '2025/2026';
}

function pluralizeYears(n) {
  const abs = Math.abs(n) % 100;
  const n1 = abs % 10;
  if (abs > 10 && abs < 20) return `${n} років`;
  if (n1 > 1 && n1 < 5) return `${n} роки`;
  if (n1 === 1) return `${n} рік`;
  return `${n} років`;
}

function computeExperienceLabel(startYear) {
  if (!startYear) return '—';
  const now = new Date().getFullYear();
  const diff = Math.max(0, now - parseInt(startYear));
  return pluralizeYears(diff);
}
...
(function immediateTabRestore() {
  ...
  if (savedTab && document.getElementById(savedTab)) {
    ...
    const tabMap = {
      'tab-journal': 0, 'tab-attendance': 1, 'tab-lessons': 2, 'tab-groups': 3, 'tab-students': 4, 'tab-subjects': 5
    };
...
// ========== МОДУЛІ ==========
const MODULE_TYPE = 'Модуль';

// Хронологічний порядок: за датою; в один день модуль стоїть ПІСЛЯ звичайних занять
function sortLessonsChrono(lessons) {
  return lessons.sort((a, b) => {
    if (a.date !== b.date) return (a.date || '').localeCompare(b.date || '');
    const ra = a.type === MODULE_TYPE ? 1 : 0, rb = b.type === MODULE_TYPE ? 1 : 0;
...
// Округлення до цілого: 2.5 -> 3, 2.4 -> 2
function roundModuleAverage(total, count) {
  if (count <= 0) return null;
  return Math.floor(total / count + 0.5 + 1e-9);
}

function gradeClassForNumber(n) {
...
function isNumericGrade(g) {
  return !!g && g !== 'н' && g !== 'св' && !isNaN(parseFloat(g));
}

// lessons — уже відсортовані хронологічно; gradeOf(lessonId) -> оцінка студента
// Повертає { [moduleLessonId]: ціле число | null }
function computeModuleValues(lessons, gradeOf) {
  const result = {};
  let total = 0, count = 0;
  lessons.forEach(l => {
    if (l.type === MODULE_TYPE) {
      result[l.id] = roundModuleAverage(total, count);
      total = 0; count = 0;
    } else {
      const g = gradeOf(l.id);
      if (isNumericGrade(g)) { total += parseFloat(g); count++; }
    }
  });
  return result;
}
...
function updateGradeCellDisplay(cell, grade, scale) {
  ...
  recalcModuleCellsInRow(cell.closest('tr'));
...
// Миттєвий перерахунок клітинок «Модуль» у рядку студента (з даних у DOM)
function recalcModuleCellsInRow(row) {
  if (!row) return;
  let total = 0, count = 0;
  row.querySelectorAll('td.grade-cell, td.module-cell').forEach(td => {
    if (td.classList.contains('module-cell')) {
      const v = roundModuleAverage(total, count);
      td.className = 'module-cell ' + (v !== null ? gradeClassForNumber(v) : '');
      td.textContent = v !== null ? v : '—';
      total = 0; count = 0;
    } else {
      const g = td.dataset.currentGrade || '';
      if (isNumericGrade(g)) { total += parseFloat(g); count++; }
    }
  });
}
...
async function updateStudentStats(studentId) {
  ...
  sortLessonsChrono(lessons);
  
  const hasModules = lessons.some(l => l.type === MODULE_TYPE);
  let totalPoints = 0, gradeCount = 0, nCount = 0, svCount = 0;
  for (const lesson of lessons) {
      if (lesson.type === MODULE_TYPE) continue;
  ...
  if (row) {
      const avgCell = row.querySelector('.stat-avg-cell');
      if (avgCell && !hasModules) {
          let avgClass = '';
          if (!isNaN(avgNum)) {
          ...
          avgCell.className = `stat-avg-cell stat-cell ${avgClass}`;
          avgCell.textContent = avg;
      }
      const nCell = row.querySelector('.stat-n-cell');
      if (nCell) { 
          nCell.className = 'stat-n-cell ' + (nCount > 0 ? 'cell-h-active' : 'stat-cell-red-text'); 
          nCell.textContent = nCount; 
      }
      
      const svCell = row.querySelector('.stat-sv-cell');
      if (svCell) { 
          svCell.className = 'stat-sv-cell ' + (svCount > 0 ? 'cell-sv-active' : 'stat-cell-red-text'); 
          svCell.textContent = svCount; 
      }
  }
}
...
function syncAttendanceSelects() {
  const g = document.getElementById("journalGroupSelect"), sb = document.getElementById("journalSubjectSelect");
  const ag = document.getElementById("attGroupSelect"), asb = document.getElementById("attSubjectSelect");
  if (!g || !sb || !ag || !asb) return;
  ag.innerHTML = g.innerHTML; ag.value = g.value;
  asb.innerHTML = sb.innerHTML; asb.value = sb.value;
}

function attTypeShort(type, number) {
  switch (type) {
    case 'Лекція': return 'Лек.';
    case 'Практичне заняття': return 'Прак.';
    case 'Лабораторне заняття': return 'Лаб.';
    case 'Семінарське заняття': return 'Сем.';
    case 'Самостійне вивчення': return 'С.в.';
    case 'Залік з модуля': return 'Залік';
    case 'Тематичне оцінювання': return 'Тем.';
    default: return type;
  }
}

// Відвідування зберігається в ОКРЕМОМУ полі "attendance" того ж документа grades,
// щоб не перезаписувати числову оцінку (студент міг здати роботу, але бути відсутнім на парі).
async function saveAttendance(lessonId, studentId, value) {
  const q = query(collection(db, "grades"), where("lessonId", "==", lessonId), where("studentId", "==", studentId));
  const existing = await getDocs(q);
  if (existing.empty) {
    if (value === '') return;
    await addDoc(collection(db, "grades"), { lessonId, studentId, attendance: value, updatedAt: new Date().toISOString() });
    return;
  }
  ...
  if (value === '') {
    if (data.grade === undefined || data.grade === null || data.grade === '') {
      ...
      await updateDoc(doc(db, "grades", docId), { attendance: deleteField(), updatedAt: new Date().toISOString() });
    }
  } else {
    await updateDoc(doc(db, "grades", docId), { attendance: value, updatedAt: new Date().toISOString() });
  }
}

function updateAttendanceRowCounts(row) {
  if (!row) return;
  let n = 0, sv = 0;
  row.querySelectorAll('.att-cell').forEach(c => {
    if (c.dataset.value === 'н') n++;
    else if (c.dataset.value === 'св') sv++;
  });
  const nCell = row.querySelector('.att-n'), svCell = row.querySelector('.att-sv');
  if (nCell) { nCell.className = 'att-n ' + (n > 0 ? 'cell-h-active' : 'stat-cell-red-text'); nCell.textContent = n; }
  if (svCell) { svCell.className = 'att-sv ' + (sv > 0 ? 'cell-sv-active' : 'stat-cell-red-text'); svCell.textContent = sv; }
}
...
async function renderAttendance() {
  ...
  const groupId = document.getElementById("journalGroupSelect").value;
  const subjectId = document.getElementById("journalSubjectSelect").value;
  const empty = '<div class="empty-state">Оберіть групу та предмет/дисципліну</div>';
  if (!currentAcademicYear || !groupId || !subjectId || groupId === "Оберіть групу" || subjectId === "Оберіть предмет") {
  ...
  if (students.length === 0) { container.innerHTML = '<div class="empty-state">Немає студентів у групі</div>'; return; }

  const lessonsSnap = await getDocs(query(collection(db, "lessons"), where("subjectId", "==", subjectId), where("teacherId", "==", currentTeacherId)));
  const lessons = [];
  lessonsSnap.forEach(l => { const d = { id: l.id, ...l.data() }; if (d.type !== MODULE_TYPE) lessons.push(d); });
  sortLessonsChrono(lessons);
  if (lessons.length === 0) {
    container.innerHTML = '<div class="empty-state">Немає занять з цього предмета. Додайте заняття у вкладці "Заняття"</div>';
    return;
  }

  const allRecords = {};
  for (const lesson of lessons) {
    const gs = await getDocs(query(collection(db, "grades"), where("lessonId", "==", lesson.id)));
    gs.forEach(g => {
      const d = g.data();
      if (!allRecords[lesson.id]) allRecords[lesson.id] = {};
      allRecords[lesson.id][d.studentId] = { grade: d.grade, attendance: d.attendance };
    });
  }
  if (token !== attendanceRenderToken) return; // з'явився новіший запит

  let html = '<table class="journal-table attendance-table"><thead><tr><th>№</th><th>Студент</th>';
  lessons.forEach(lesson => {
    const p = lesson.date.split('-');
    const dateStr = `${p[2]}.${p[1]}.${p[0].slice(-2)}`;
    const isSpecial = (lesson.type === 'Залік з модуля' || lesson.type === 'Тематичне оцінювання');
    html += `<th class="date-cell ${isSpecial ? 'lesson-special' : ''}" data-lesson-type="${lesson.type}" data-lesson-topic="${lesson.topic || ''}" data-lesson-date="${lesson.date}">${dateStr}<br><small>${attTypeShort(lesson.type)}</small></th>`;
  });
  html += '<th class="stat-header-red">Н</th><th class="stat-header-red">СВ</th></tr></thead><tbody>';

  let i = 1;
  for (const st of students) {
    let n = 0, sv = 0, cells = '';
    for (const lesson of lessons) {
      const rec = allRecords[lesson.id]?.[st.id] || {};
      // Сумісність зі старими записами, де н/св зберігались прямо в grade
      const mark = rec.attendance || ((rec.grade === 'н' || rec.grade === 'св') ? rec.grade : '');
      const hasGrade = isNumericGrade(rec.grade);
      if (mark === 'н') { n++; cells += `<td class="att-cell grade-n" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="н" data-has-grade="${hasGrade}" data-grade-value="${rec.grade || ''}"${hasGrade ? ` title="Є оцінка: ${rec.grade}"` : ''}>н</td>`; }
      else if (mark === 'св') { sv++; cells += `<td class="att-cell grade-sv" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="св" data-has-grade="${hasGrade}" data-grade-value="${rec.grade || ''}"${hasGrade ? ` title="Є оцінка: ${rec.grade}"` : ''}>св</td>`; }
      else { cells += `<td class="att-cell ${hasGrade ? 'att-cell-graded' : ''}" data-lesson-id="${lesson.id}" data-student-id="${st.id}" data-value="" data-has-grade="${hasGrade}" data-grade-value="${rec.grade || ''}"${hasGrade ? ` title="Є оцінка: ${rec.grade}. Клікніть, щоб додатково позначити н/св"` : ''}>${hasGrade ? rec.grade : '—'}</td>`; }
    }
    html += `<tr><td style="text-align:center; width:40px;">${i++}</td><td style="text-align:left"><strong>${escapeHtml(st.name)}</strong> ${st.role === 'prefect' ? '⭐' : ''}</td>${cells}` +
      `<td class="att-n ${n > 0 ? 'cell-h-active' : 'stat-cell-red-text'}">${n}</td><td class="att-sv ${sv > 0 ? 'cell-sv-active' : 'stat-cell-red-text'}">${sv}</td></tr>`;
  }
  html += '</tbody></table>';
  ...
  container.querySelectorAll('.date-cell').forEach(cell => {
    ...
    cell.addEventListener('touchstart', (e) => {
      e.stopPropagation();
      if (currentTooltip && currentTooltip.dataset.forCell === cell.dataset.lessonDate + cell.dataset.lessonTopic) { hideTooltip(); return; }
      showTooltip(e);
...
// Клік по клітинці: порожньо -> н -> св -> порожньо
document.getElementById("attendanceTableContainer")?.addEventListener('click', (e) => {
  const cell = e.target.closest('.att-cell');
  if (!cell) return;
  const order = ['', 'н', 'св'];
  const next = order[(order.indexOf(cell.dataset.value || '') + 1) % order.length];
  const hasGrade = cell.dataset.hasGrade === 'true';

  cell.dataset.value = next;
  if (next === 'н') { cell.className = 'att-cell grade-n'; cell.textContent = 'н'; }
  else if (next === 'св') { cell.className = 'att-cell grade-sv'; cell.textContent = 'св'; }
  else { cell.className = 'att-cell' + (hasGrade ? ' att-cell-graded' : ''); cell.textContent = hasGrade ? cell.dataset.gradeValue : '—'; }
  cell.title = hasGrade ? `Є оцінка: ${cell.dataset.gradeValue}` : '';

  updateAttendanceRowCounts(cell.closest('tr'));
  const lessonId = cell.dataset.lessonId, studentId = cell.dataset.studentId;
  attendanceQueue = attendanceQueue
    .then(() => saveAttendance(lessonId, studentId, next))
    .catch(err => { console.error('Помилка збереження відвідування:', err); alert('Не вдалося зберегти відвідування. Спробуйте ще раз.'); });
});

document.getElementById("attGroupSelect")?.addEventListener("change", async (e) => {
  const g = document.getElementById("journalGroupSelect");
  g.value = e.target.value;
  if (g.value) await populateJournalSubjects(g.value);
  else document.getElementById("journalSubjectSelect").innerHTML = '<option value="">Оберіть предмет</option>';
  saveLastSelection();
  syncAttendanceSelects();
  renderAttendance();
});
document.getElementById("attSubjectSelect")?.addEventListener("change", (e) => {
  document.getElementById("journalSubjectSelect").value = e.target.value;
  saveLastSelection();
  renderAttendance();
});
...
async function renderJournal() {
  ...
  if (!year || !groupId || !subjectId || groupId === "Оберіть групу" || subjectId === "Оберіть предмет") {
      document.getElementById("journalTableContainer").innerHTML = '<div class="empty-state">Оберіть групу та предмет/дисципліну</div>';
      return;
  }
  ...
  students.forEach(student => {
      let totalPoints = 0, gradeCount = 0, nCount = 0, svCount = 0;
      lessons.forEach(lesson => {
          if (lesson.type === MODULE_TYPE) return;
  ...
  const moduleValues = {};
  students.forEach(st => {
      moduleValues[st.id] = computeModuleValues(lessons, id => allGrades[id]?.[st.id]);
  });
  ...
  lessons.forEach(lesson => {
      ...
      switch (lesson.type) {
          ...
          case 'Модуль': typeShort = 'Мод. №' + (lesson.number || ''); break;
          default: typeShort = lesson.type;
      }
      const isSpecial = (lesson.type === 'Залік з модуля' || lesson.type === 'Тематичне оцінювання');
      const specialClass = isSpecial ? 'lesson-special' : '';
      const isModule = lesson.type === MODULE_TYPE;
    const tipType = isModule ? `Модуль №${lesson.number || ''}` : lesson.type;
    const tipTopic = isModule ? 'Середня оцінка (округлена) за заняття цього модуля' : (lesson.topic || '');
    const thClass = isModule ? 'lesson-module' : specialClass;
    html += `<th class="date-cell ${thClass}" data-lesson-type="${tipType}" data-lesson-topic="${tipTopic}" data-lesson-date="${lesson.date}">${dateStr}<br><small>${typeShort}</small></th>`;
  });
  
  const isExam = (subjectControl === 'екзамен');
  const finalColHeader = isExam ? 'Екзамен' : 'Підсумкова';
  html += `${hasModules ? '' : '<th class="stat-cell">Сер.</th>'}<th class="stat-header-red">Н</th><th class="stat-header-red">СВ</th><th class="stat-cell">${finalColHeader}</th></tr></thead><tbody>`;
  ...
  for (const student of students) {
      ...
      for (const lesson of lessons) {
          if (lesson.type === MODULE_TYPE) {
              const mv = moduleValues[student.id][lesson.id];
              html += `<td class="module-cell ${mv !== null ? gradeClassForNumber(mv) : ''}" data-module-id="${lesson.id}" data-student-id="${student.id}">${mv !== null ? mv : '—'}</td>`;
              continue;
          }
          const grade = allGrades[lesson.id]?.[student.id] || '';
          let gradeClass = '';
      ...
      // Стилізація для Н та СВ
      let nClass = stats.n > 0 ? 'cell-h-active' : 'stat-cell-red-text';
      let svClass = stats.sv > 0 ? 'cell-sv-active' : 'stat-cell-red-text';

      if (!hasModules) {
          html += `<td class="stat-avg-cell stat-cell ${avgClass}">${stats.avg}</td>`;
      }
      html += `<td class="stat-n-cell ${nClass}">${stats.n}</td>
               <td class="stat-sv-cell ${svClass}">${stats.sv}</td>`;
  ...
  document.querySelectorAll('.date-cell').forEach(cell => {
      cell.addEventListener('mouseenter', showTooltip);
      cell.addEventListener('mouseleave', hideTooltip);
      // На дотикових екранах (планшети/телефони) hover недоступний,
      // тож дозволяємо показати підказку по тапу.
      cell.addEventListener('touchstart', (e) => {
          e.stopPropagation();
          if (currentTooltip && currentTooltip.dataset.forCell === cell.dataset.lessonDate + cell.dataset.lessonTopic) {
              hideTooltip();
...
// Ховаємо підказку при тапі будь-де поза коміркою дати (для тач-пристроїв)
...
async function updateLesson(id) {
  ...
  const currentData = currentLessonSnap.exists() ? currentLessonSnap.data() : {};
  let currentNumber = currentData.number || null;
  // якщо змінили вид заняття або предмет — лічильник рахуємо заново
  if (currentData.type !== type || currentData.subjectId !== subjectId) {
    currentNumber = await getNextLessonNumberForType(type, subjectId);
  }
  const data = {
    subjectId: subjectId,
    type: type,
    date: date,
    topic: document.getElementById("lessonType").value === MODULE_TYPE ? '' : document.getElementById("lessonTopic").value,
...
function toggleLessonTopicField() {
  const isModule = document.getElementById("lessonType").value === MODULE_TYPE;
  const g = document.getElementById("lessonTopicGroup");
  if (g) g.style.display = isModule ? "none" : "";
}
document.getElementById("lessonType")?.addEventListener("change", toggleLessonTopicField);

document.getElementById("lessonSubject").addEventListener("change", async (e) => {
await updateSectionSelect(e.target.value);
});
...
async function loadMyLessons() {
  ...
  snap.forEach(docSnap => {
    ...
    if (currentAcademicYear && subMap[data.subjectId] && subMap[data.subjectId].year && subMap[data.subjectId].year !== currentAcademicYear) return;
    lessonsArray.push({
      id: docSnap.id,
      ...data,
  ...
  for (const section of sectionsList) {
    ...
    for (const l of section.lessons) {
      ...
      switch (l.type) {
        ...
        case 'Залік з модуля': typeShort = 'Залік'; break;
        case 'Тематичне оцінювання': typeShort = 'Тем.'; break;
        case 'Модуль': typeShort = 'Модуль'; break;
      ...
      html += `<div class="data-card">
                  <div class="data-info">
                      ...
                      <div>
                          <div class="data-title">${l.type === MODULE_TYPE ? 'Модуль №' + (l.number || '') : (escapeHtml(l.topic) || "Без теми")}</div>
...
async function loadGroups() {
  ...
  snap.forEach(docSnap => {
    ...
    const year = g.academicYear || 'Не вказано';
    if (currentAcademicYear && year !== currentAcademicYear) return;
    const course = g.course || '1';
    if (course !== currentGroupsCourse) return;
    groups.push({ id: docSnap.id, name: g.name, year, course, studentCount: studentCountByGroup[docSnap.id] || 0 });
  });
  if (groups.length === 0) {
    container.innerHTML = `<div class="empty-state">Груп не знайдено на ${currentGroupsCourse} курсі${currentAcademicYear ? ` за ${currentAcademicYear} н.р.` : ''}</div>`;
    populateSelects();
    return;
  }
  ...
  groups.forEach(g => {
    html += `<div class="data-card" onclick="window.showGroupStats('${g.id}','${g.name}')"><div class="data-info"><div class="data-icon">👥</div><div><div class="data-title">${g.name}</div><div class="data-sub">Студентів: ${g.studentCount}<span class="course-badge">${g.course} курс</span></div></div></div><div class="data-actions"><button class="icon-btn edit-btn" onclick="event.stopPropagation(); window.editGroup('${g.id}','${g.name}','${g.year}','${g.course}')">✏️</button><button class="icon-btn delete-btn" onclick="event.stopPropagation(); window.deleteItem('groups','${g.id}')">🗑️</button></div></div>`;
  });
  html += '</div>';
  container.innerHTML = html;
  populateSelects();
...
window.setGroupsCourse = function (course) {
  currentGroupsCourse = course;
  localStorage.setItem(COURSE_FILTER_KEYS.groups, course);
  document.querySelectorAll('#groupsCourseTabs .subtab-btn').forEach(b => b.classList.toggle('active', b.dataset.course === course));
  loadGroups();
};

window.setSubjectsCourse = function (course) {
  currentSubjectsCourse = course;
  localStorage.setItem(COURSE_FILTER_KEYS.subjects, course);
  document.querySelectorAll('#subjectsCourseTabs .subtab-btn').forEach(b => b.classList.toggle('active', b.dataset.course === course));
  loadMySubjects();
};

window.editGroup = (id, name, year, course) => {
  currentEditId = id;
  document.getElementById("groupName").value = name;
  document.getElementById("groupYear").value = year || getDefaultAcademicYear();
  document.getElementById("groupCourse").value = course || '1';
  ...
  btn.onclick = async () => {
    ...
    const newCourse = document.getElementById("groupCourse").value;
    if (newName) {
      await updateDoc(doc(db, "groups", id), { name: newName, academicYear: newYear, course: newCourse });
      resetGroupForm();
      loadGroups();
      updateStats();
    }
...
function resetGroupForm() {
  ...
  document.getElementById("groupYear").value = getDefaultAcademicYear();
  document.getElementById("groupCourse").value = currentGroupsCourse || "1";
  currentEditId = null;
  const btn = document.getElementById("saveGroupBtn");
  btn.innerText = "Зберегти";
  btn.onclick = async () => {
      ...
      if (name) {
          await addGroupAndUpdateSelects(name, academicYear, course);
      }
  };
...
async function addGroupAndUpdateSelects(groupName, academicYear, course) {
  await addDoc(collection(db, "groups"), { name: groupName, academicYear, course: course || '1' });
...
async function loadStudents() {
  ...
  groupsSnap.forEach(d => {
    ...
    if (!currentAcademicYear || group.academicYear === currentAcademicYear) groupsArray.push(group);
  });
  ...
  groupsArray.forEach(g => { groupOptions += `<option value="${g.id}">${g.name}</option>`; });
  ...
  const currentYearGroupIds = new Set(groupsArray.map(g => g.id));
  let students = [];
  snap.forEach(docSnap => { students.push({ id: docSnap.id, ...docSnap.data() }); });
  students = students.filter(s => currentYearGroupIds.has(s.groupId));
  ...
  if (students.length === 0) { container.innerHTML = `<div class="empty-state">Студентів не знайдено${currentAcademicYear ? ` за ${currentAcademicYear} н.р.` : ''}</div>`; return; }
  let html = '<div class="data-list">';
  students.forEach(s => {
    ...
    html += `<div class="data-card" onclick="window.showStudentStats('${s.id}','${safeName.replace(/'/g, "\\'")}')"><div class="data-info"><div class="data-icon">👨‍🎓</div><div><div class="data-title">${safeName} ${s.role === 'prefect' ? '<span class="prefect-badge">Староста</span>' : ''} ${s.userId ? '<span class="course-badge" title="Студент увійшов у власний кабінет">✅ Активовано</span>' : ''}</div><div class="data-sub">${groupName} | ${academicYear} | ${s.phone || ''}</div></div></div><div class="data-actions"><button class="icon-btn edit-btn" onclick="event.stopPropagation(); window.editStudent('${s.id}','${s.name}','${s.groupId}','${s.phone || ''}','${s.role || 'student'}')">✏️</button><button class="icon-btn delete-btn" onclick="event.stopPropagation(); window.deleteItem('students','${s.id}')">🗑️</button></div></div>`;
  });
  html += '</div>';
  container.innerHTML = html;
}

window.showStudentStats = async (studentId, studentName) => {
  const modal = document.getElementById("studentModal");
  const modalBody = document.getElementById("studentModalBody");
  document.getElementById("studentModalTitle").innerHTML = `Статистика студента: ${escapeHtml(studentName)}`;
  ...
  try {
    ...
    if (!studentSnap.exists()) {
      modalBody.innerHTML = '<div class="stat-item">Студента не знайдено</div>';
      return;
    }
    const groupId = studentSnap.data().groupId;
    const studentData = studentSnap.data();

    const personalInfoRows = [];
    if (studentData.birthDate) {
      const dp = studentData.birthDate.split('-');
      personalInfoRows.push(`<div class="hours-breakdown-item"><span class="hours-breakdown-label">Дата народження</span><span class="hours-breakdown-value">${dp.length === 3 ? `${dp[2]}.${dp[1]}.${dp[0]}` : studentData.birthDate}</span></div>`);
    }
    if (studentData.admissionYear) {
      personalInfoRows.push(`<div class="hours-breakdown-item"><span class="hours-breakdown-label">Рік вступу до коледжу</span><span class="hours-breakdown-value">${studentData.admissionYear}</span></div>`);
    }
    if (studentData.phone) {
      personalInfoRows.push(`<div class="hours-breakdown-item"><span class="hours-breakdown-label">Номер телефону</span><span class="hours-breakdown-value">${escapeHtml(studentData.phone)}</span></div>`);
    }
    const personalInfoHtml = personalInfoRows.length > 0
      ? `<div class="stats-card" style="margin-bottom:16px; margin-top:0;"><h3 style="font-family: 'Unbounded', sans-serif; font-size: 14px; margin-bottom: 4px;">🧑‍🎓 Особиста інформація</h3><div class="hours-breakdown">${personalInfoRows.join('')}</div></div>`
      : `<div class="journal-info" style="margin-bottom:16px;">Студент ще не заповнив особисту інформацію (дата народження, рік вступу, телефон) у своєму кабінеті.</div>`;
    ...
    if (subjects.length === 0) {
      modalBody.innerHTML = personalInfoHtml + '<div class="stat-item">Немає предметів/дисциплін для цього студента</div>';
      return;
    }

    const rows = [];
    for (const subject of subjects) {
      const finalSnap = await getDocs(query(collection(db, "finalGrades"), where("studentId", "==", studentId), where("subjectId", "==", subject.id)));
      let gradeText = '—';
      ...
      if (!finalSnap.empty && finalSnap.docs[0].data().grade) {
        ...
        const lessonsSnap = await getDocs(query(collection(db, "lessons"), where("subjectId", "==", subject.id), where("teacherId", "==", currentTeacherId)));
        let total = 0, count = 0;
        for (const lessonDoc of lessonsSnap.docs) {
          const gradeSnap = await getDocs(query(collection(db, "grades"), where("lessonId", "==", lessonDoc.id), where("studentId", "==", studentId)));
          if (!gradeSnap.empty) {
      ...
      rows.push(`<div class="stat-item"><div class="stat-label">${escapeHtml(subject.name)} — ${label}</div><div class="stat-value">${gradeText}</div></div>`);
    }
    modalBody.innerHTML = personalInfoHtml + rows.join('');
  } catch (e) {
    console.error(e);
    modalBody.innerHTML = '<div class="stat-item">Сталася помилка при завантаженні статистики</div>';
  }
};

window.closeStudentModal = () => { document.getElementById("studentModal").classList.remove("active"); };

async function getGroupAcademicYear(groupId) {
  if (!groupId) return getDefaultAcademicYear();
  const groupDoc = await getDoc(doc(db, "groups", groupId));
  return groupDoc.exists() ? (groupDoc.data().academicYear || getDefaultAcademicYear()) : getDefaultAcademicYear();
}

window.editStudent = (id, name, groupId, phone, role) => {
  currentEditId = id;
  ...
  btn.innerText = "Оновити";
  btn.onclick = async () => {
    const newGroupId = document.getElementById("studentGroup").value;
    const data = {
      name: document.getElementById("studentName").value,
      groupId: newGroupId,
      academicYear: await getGroupAcademicYear(newGroupId),
...
function resetStudentForm() {
  ...
  // Відновлення вибраної групи для наступного студента
  const lastGroup = sessionStorage.getItem(STORAGE_KEYS.lastStudentGroup);
  if (lastGroup && document.querySelector(`#studentGroup option[value="${lastGroup}"]`)) {
      document.getElementById("studentGroup").value = lastGroup;
  }
...
document.getElementById("saveStudentBtn").onclick = async () => {
  ...
  const data = {
    name: document.getElementById("studentName").value,
    groupId: groupId,
    academicYear: await getGroupAcademicYear(groupId),
    phone: document.getElementById("studentPhone").value,
    role: document.getElementById("studentRole").value
  };
...
function renderSubjectCard(id, s, groupName) {
  return `<div class="data-card" onclick="window.showSubjectInfo('${id}')"><div class="data-info"><div class="data-icon">📘</div><div><div class="data-title">${escapeHtml(s.name || '')}</div><div class="data-sub">${escapeHtml(groupName)} | ${s.academicYear || 'Не вказано'}</div></div></div><div class="data-actions"><button class="icon-btn edit-btn" onclick="event.stopPropagation(); window.editSubject('${id}')">✏️</button><button class="icon-btn delete-btn" onclick="event.stopPropagation(); window.deleteItem('subjects','${id}')">🗑️</button></div></div>`;
}
...
async function loadMySubjects() {
  ...
  const groupsSnap = await getDocs(collection(db, "groups"));
  const groupsMap = {};
  groupsSnap.forEach(g => { groupsMap[g.id] = { name: g.data().name, course: g.data().course || '1' }; });
  const container = document.getElementById("subjectsList");
  const docs = snap.docs.filter(docSnap => {
    const s = docSnap.data();
    const year = s.academicYear || 'Не вказано';
    return !currentAcademicYear || year === currentAcademicYear;
  });

  // Педагогічне навантаження рахуємо по всіх предметах поточного навчального року,
  // незалежно від того, який курс зараз обраний у фільтрі
  updateWorkloadDisplay(docs.map(d => d.data()));

  if (docs.length === 0) {
    container.innerHTML = `<div class="empty-state">Предметів/дисциплін не знайдено${currentAcademicYear ? ` за ${currentAcademicYear} н.р.` : ''}. Додайте ваш перший предмет!</div>`;
    populateSelects();
    return;
  }

  const activeCourse = currentSubjectsCourse || '1';
  const byGroup = {};
  const noGroup = [];

  docs.forEach(docSnap => {
    const s = docSnap.data();
    const groupInfo = groupsMap[s.groupId];
    if (!groupInfo) { noGroup.push({ id: docSnap.id, ...s }); return; }
    if (groupInfo.course !== activeCourse) return;
    if (!byGroup[s.groupId]) byGroup[s.groupId] = { name: groupInfo.name, subjects: [] };
    byGroup[s.groupId].subjects.push({ id: docSnap.id, ...s });
  });

  const groupIds = Object.keys(byGroup).sort((a, b) => byGroup[a].name.localeCompare(byGroup[b].name));

  let html = '';
  if (groupIds.length === 0 && noGroup.length === 0) {
    html = `<div class="empty-state">На ${activeCourse} курсі предметів/дисциплін не знайдено.</div>`;
  } else {
    groupIds.forEach(gid => {
      const grp = byGroup[gid];
      grp.subjects.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      html += `<div class="section-header-wrapper"><div class="section-header-title">📁 ${escapeHtml(grp.name)} <span class="group-count">(${grp.subjects.length})</span></div></div>`;
      html += '<div class="data-list">';
      grp.subjects.forEach(s => { html += renderSubjectCard(s.id, s, grp.name); });
      html += '</div>';
    });

    if (noGroup.length > 0) {
      noGroup.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      html += `<div class="section-header-wrapper"><div class="section-header-title">📁 Без прив'язки до групи <span class="group-count">(${noGroup.length})</span></div></div>`;
      html += '<div class="data-list">';
      noGroup.forEach(s => { html += renderSubjectCard(s.id, s, s.groupName || 'Групу не обрано'); });
      html += '</div>';
    }
  }

  container.innerHTML = html || `<div class="empty-state">Немає предметів на цьому курсі.</div>`;
  populateSelects();
}
...
async function loadGroupsToSelect(selectElement) {
  ...
  const groupsArray = [];
  snap.forEach(doc => {
      const year = doc.data().academicYear || 'Не вказано';
      if (currentAcademicYear && year !== currentAcademicYear) return; // Фільтр за роком
      groupsArray.push({
          id: doc.id,
          name: doc.data().name,
          academicYear: year,
          course: doc.data().course || '1'
      });
  });
  ...
  groupsArray.forEach(group => {
      options += `<option value="${group.id}">${group.name} (${group.course} курс, ${group.academicYear})</option>`;
  });
...
async function updateStats() {
  ...
  const currentYearGroupIds = new Set();
  let groupsCount = 0;
  groupsSnap.forEach(g => {
    const year = g.data().academicYear || 'Не вказано';
    if (!currentAcademicYear || year === currentAcademicYear) {
      groupsCount++;
      currentYearGroupIds.add(g.id);
    }
  });

  let studentsCount = 0;
  studentsSnap.forEach(s => { if (currentYearGroupIds.has(s.data().groupId)) studentsCount++; });

  let subjectsCount = 0;
  subjectsSnap.forEach(s => {
    const year = s.data().academicYear || 'Не вказано';
    if (!currentAcademicYear || year === currentAcademicYear) subjectsCount++;
  });

  document.getElementById("statStudents").innerText = studentsCount;
  document.getElementById("statGroups").innerText = groupsCount;
  document.getElementById("statSubjects").innerText = subjectsCount;
}
...
async function populateJournalSubjects(groupId, preserveSubjectId) {
  const groupDoc = await getDoc(doc(db, "groups", groupId));
  const groupName = groupDoc.exists() ? groupDoc.data().name : '';
  const groupYear = groupDoc.exists() ? groupDoc.data().academicYear : '';
  const subjectsSnap = await getDocs(query(collection(db, "subjects"), where("groupId", "==", groupId), where("teacherId", "==", currentTeacherId)));
  ...
  subjectsSnap.forEach(docSnap => {
    ...
    subjectsArray.push({ id: docSnap.id });
  });
  document.getElementById("journalSubjectSelect").innerHTML = subjectOptions;
  if (preserveSubjectId && subjectsArray.some(s => s.id === preserveSubjectId)) {
    document.getElementById("journalSubjectSelect").value = preserveSubjectId;
  } else if (subjectsArray.length === 1) {
    document.getElementById("journalSubjectSelect").value = subjectsArray[0].id;
  }
}
...
async function populateJournalFilters() {
  document.getElementById("journalGroupSelect").onchange = async () => {
    ...
    await populateJournalSubjects(selectedGroup);
    saveLastSelection();
    renderJournal();
  };
  ...
  if (currentAcademicYear) {
    const groupsSnap = await getDocs(query(collection(db, "groups"), where("academicYear", "==", currentAcademicYear)));
    groupsSnap.forEach(g => {
  ...
  const lastSubject = sessionStorage.getItem(STORAGE_KEYS.lastSubject);
  let groupToSelect = '';
  if (lastGroup && groupsArray.some(g => g.id === lastGroup)) groupToSelect = lastGroup;
  else if (groupsArray.length === 1) groupToSelect = groupsArray[0].id;

  if (groupToSelect) {
    document.getElementById("journalGroupSelect").value = groupToSelect;
    await populateJournalSubjects(groupToSelect, lastSubject);
  } else {
    document.getElementById("journalSubjectSelect").innerHTML = '<option value="">Оберіть предмет</option>';
  }
...
function populateSelects() {
  ...
  getDocs(query(collection(db, "subjects"), where("teacherId", "==", currentTeacherId))).then(async snap => {
      ...
      for (const docSnap of snap.docs) {
          ...
          if (currentAcademicYear && (academicYear || s.academicYear) && (academicYear || s.academicYear) !== currentAcademicYear) continue;
...
// ========== СПІЛЬНІ НАВЧАЛЬНІ РОКИ ==========
function yearDocId(yearStr) {
  return yearStr.replace(/\//g, '_');
}

async function ensureAcademicYearsSeed() {
  const snap = await getDocs(collection(db, "academicYears"));
  if (!snap.empty) return;
  const seedYears = ["2025/2026", "2026/2027"];
  for (const y of seedYears) {
    const startYear = parseInt(y.split('/')[0]);
    await setDoc(doc(db, "academicYears", yearDocId(y)), { year: y, startYear, createdAt: new Date().toISOString() });
  }
}

async function loadAcademicYears() {
  await ensureAcademicYearsSeed();
  const snap = await getDocs(collection(db, "academicYears"));
  const years = [];
  snap.forEach(d => years.push(d.data().year || d.id.replace(/_/g, '/')));
  years.sort((a, b) => parseInt(b.split('/')[0]) - parseInt(a.split('/')[0]));
  academicYearsCache = years;
  return years;
}

function buildAcademicYearOptionsHtml(includePlaceholder, placeholderText) {
  let html = includePlaceholder ? `<option value="">${placeholderText || 'Оберіть рік'}</option>` : '';
  academicYearsCache.forEach(y => { html += `<option value="${y}">${y}</option>`; });
  return html;
}

function populateAcademicYearSelects() {
  ['groupYear'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const prevValue = el.value;
    el.innerHTML = buildAcademicYearOptionsHtml(false);
    if (prevValue && academicYearsCache.includes(prevValue)) el.value = prevValue;
    else el.value = getDefaultAcademicYear();
  });
}

function populateCareerYearSelects() {
  const nowYear = new Date().getFullYear();
  let html = '';
  for (let y = nowYear; y >= 1965; y--) {
    html += `<option value="${y}">${y}</option>`;
  }
  ['regStartYear', 'piStartYear'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const isReg = id === 'regStartYear';
    el.innerHTML = (isReg ? '<option value="">Оберіть рік</option>' : '') + html;
  });
  const newYearStartEl = document.getElementById('newYearStart');
  if (newYearStartEl) {
    let optHtml = '';
    for (let y = nowYear + 2; y >= 2015; y--) {
      optHtml += `<option value="${y}">${y}/${y + 1}</option>`;
    }
    newYearStartEl.innerHTML = optHtml;
    updateNewYearEndPreview();
  }
}

function updateNewYearEndPreview() {
  const sel = document.getElementById('newYearStart');
  const endEl = document.getElementById('newYearEnd');
  if (!sel || !endEl) return;
  const start = parseInt(sel.value);
  endEl.textContent = start ? `${start}/${start + 1}` : '—';
}

function isProfileComplete(u) {
  return !!(u && u.profileCompleted && u.lastName && u.firstName && u.startYear && u.qualificationCategory);
}

// ========== МАЙСТЕР РЕЄСТРАЦІЇ ВИКЛАДАЧА ==========
let wizardIsFirstTime = true;

function openRegistrationModal() {
  wizardStep = 1;
  const u = currentUserData || {};
  wizardIsFirstTime = !u.lastName && !u.firstName && !u.startYear && !u.qualificationCategory;
  document.getElementById("regLastName").value = u.lastName || "";
  document.getElementById("regFirstName").value = u.firstName || "";
  document.getElementById("regMiddleName").value = u.middleName || "";
  document.getElementById("regStartYear").value = u.startYear || "";
  document.getElementById("regCategory").value = u.qualificationCategory || "";
  document.getElementById("regTitle").value = u.pedagogicalTitle || "";
  applyWizardStep();
...
function applyWizardStep() {
  [1, 2, 3].forEach(n => {
    document.getElementById(`wizardStep${n}`).classList.toggle('active', n === wizardStep);
    const circle = document.getElementById(`stepCircle${n}`);
    circle.classList.toggle('active', n === wizardStep);
    circle.classList.toggle('done', n < wizardStep);
    circle.textContent = n < wizardStep ? '✓' : n;
  });
  document.getElementById("wizardCount").textContent = `${wizardStep} / 3`;
  document.getElementById("wizardProgress").style.width = `${wizardStep * 33.33}%`;
  document.getElementById("wizardBackBtn").style.visibility = wizardStep === 1 ? 'hidden' : 'visible';
  document.getElementById("wizardEyebrow").textContent = wizardIsFirstTime ? "Вітаємо в системі!" : "Завершіть налаштування профілю";
  const titles = { 1: 'Особисті дані', 2: 'Педагогічний досвід', 3: 'Підтвердження' };
  const descs = wizardIsFirstTime
    ? {
        1: "Розкажіть про себе, щоб ми могли персоналізувати ваш досвід",
        2: "Вкажіть ваш педагогічний стаж та кваліфікацію",
        3: "Перевірте введені дані перед початком роботи"
      }
    : {
        1: "Ми помітили, що ваш профіль ще не заповнено. Будь ласка, вкажіть особисті дані, щоб продовжити роботу",
        2: "Вкажіть ваш педагогічний стаж та кваліфікацію",
        3: "Перевірте введені дані перед початком роботи"
      };
  ...
  document.getElementById("wizardNextBtn").textContent = wizardStep === 3 ? "Розпочати роботу" : "Продовжити ›";
...
window.wizardBack = function () {
  if (wizardStep > 1) { wizardStep--; applyWizardStep(); }
};
...
window.wizardNext = async function () {
  if (wizardStep === 1) {
    ...
    if (!last || !first) { showWizardError("Вкажіть прізвище та ім'я"); return; }
    wizardStep = 2;
    applyWizardStep();
  } else if (wizardStep === 2) {
    const startYear = document.getElementById("regStartYear").value;
    const category = document.getElementById("regCategory").value;
    if (!startYear || !category) { showWizardError("Оберіть рік початку діяльності та кваліфікаційну категорію"); return; }
    document.getElementById("sumLastName").textContent = document.getElementById("regLastName").value.trim();
    document.getElementById("sumFirstName").textContent = document.getElementById("regFirstName").value.trim();
    document.getElementById("sumMiddleName").textContent = document.getElementById("regMiddleName").value.trim() || '—';
    document.getElementById("sumStartYear").textContent = startYear;
    document.getElementById("sumCategory").textContent = category;
    document.getElementById("sumTitle").textContent = document.getElementById("regTitle").value.trim() || '—';
    wizardStep = 3;
    applyWizardStep();
  } else {
...
async function finishRegistration() {
  ...
  const data = {
    ...
    displayNameFull: [lastName, firstName, middleName].filter(Boolean).join(' '),
    startYear: parseInt(document.getElementById("regStartYear").value),
    qualificationCategory: document.getElementById("regCategory").value,
    pedagogicalTitle: document.getElementById("regTitle").value.trim() || null,
    profileCompleted: true
  };
  try {
    await updateDoc(doc(db, "users", currentTeacherId), data);
    currentUserData = { ...currentUserData, ...data };
    applyUserHeaderInfo();
    document.getElementById("registrationModal").classList.remove("active");
    await openYearModal(true);
  } catch (e) {
    console.error(e);
    showWizardError("Помилка збереження. Спробуйте ще раз.");
  } finally {
    btn.disabled = false;
  }
}

// ========== МОДАЛЬНЕ ВІКНО ВИБОРУ НАВЧАЛЬНОГО РОКУ ==========
window.openYearModal = async function (mandatory) {
  yearModalMandatory = !!mandatory;
  if (academicYearsCache.length === 0) await loadAcademicYears();
  populateCareerYearSelects();
  const sel = document.getElementById("yearModalSelect");
  sel.innerHTML = buildAcademicYearOptionsHtml(false);
  if (currentAcademicYear && academicYearsCache.includes(currentAcademicYear)) {
    sel.value = currentAcademicYear;
  }
  document.getElementById("yearModalClose").style.display = yearModalMandatory ? "none" : "";
  document.getElementById("yearAddForm").style.display = "none";
  document.getElementById("yearAddToggle").style.display = "";
  document.getElementById("yearModalError").style.display = "none";
  document.getElementById("yearModal").classList.add("active");
};

window.closeYearModal = function () {
  if (yearModalMandatory) return;
  document.getElementById("yearModal").classList.remove("active");
};

window.toggleYearAddForm = function () {
  const form = document.getElementById("yearAddForm");
  const isHidden = form.style.display === "none";
  form.style.display = isHidden ? "flex" : "none";
  document.getElementById("yearAddToggle").style.display = isHidden ? "none" : "";
  updateNewYearEndPreview();
};

document.addEventListener('change', (e) => {
  if (e.target && e.target.id === 'newYearStart') updateNewYearEndPreview();
});

window.addNewAcademicYear = async function () {
  const start = parseInt(document.getElementById("newYearStart").value);
  if (!start) return;
  const yearStr = `${start}/${start + 1}`;
  if (!academicYearsCache.includes(yearStr)) {
    await setDoc(doc(db, "academicYears", yearDocId(yearStr)), { year: yearStr, startYear: start, createdAt: new Date().toISOString(), createdBy: currentTeacherId });
...
window.confirmYearSelection = async function () {
  const val = document.getElementById("yearModalSelect").value;
  if (!val) {
    const err = document.getElementById("yearModalError");
    err.textContent = "Оберіть навчальний рік";
    err.style.display = "block";
    return;
  }
  const wasMandatory = yearModalMandatory;
  currentAcademicYear = val;
  localStorage.setItem(ACADEMIC_YEAR_STORAGE_KEY, val);
  try { await updateDoc(doc(db, "users", currentTeacherId), { currentAcademicYear: val }); } catch (e) { }
  document.getElementById("yearBadgeValue").textContent = val;
  populateAcademicYearSelects();
  document.getElementById("yearModal").classList.remove("active");
  yearModalMandatory = false;

  if (wasMandatory) {
    await continueAfterAuth();
  } else {
    sessionStorage.removeItem(STORAGE_KEYS.lastGroup);
    sessionStorage.removeItem(STORAGE_KEYS.lastSubject);
    sessionStorage.removeItem(STORAGE_KEYS.lastLessonFilter);
    const lessonFilterEl = document.getElementById("lessonFilterSubject");
    if (lessonFilterEl) lessonFilterEl.value = "all";
    const studentGroupFilterEl = document.getElementById("studentGroupFilter");
    if (studentGroupFilterEl) studentGroupFilterEl.value = "all";
    const studentSearchEl = document.getElementById("studentSearchInput");
...
window.goToSubjectsTab = function () {
  document.getElementById("burgerMenu").classList.remove("open");
  window.switchTab('tab-subjects');
};

function applyUserHeaderInfo() {
  if (!currentUserData) return;
  const fullName = currentUserData.displayNameFull || currentUserData.displayName || "Викладач";
...
// ========== МОДАЛЬНЕ ВІКНО "ОСОБИСТА ІНФОРМАЦІЯ" ==========
window.openPersonalInfoModal = function () {
  document.getElementById("burgerMenu").classList.remove("open");
  if (!currentUserData) return;
  pendingAvatarFile = null;
  populateCareerYearSelects();
  document.getElementById("piLastName").value = currentUserData.lastName || "";
  document.getElementById("piFirstName").value = currentUserData.firstName || "";
  document.getElementById("piMiddleName").value = currentUserData.middleName || "";
  const startYearEl = document.getElementById("piStartYear");
  if (currentUserData.startYear && startYearEl.querySelector(`option[value="${currentUserData.startYear}"]`)) {
    startYearEl.value = currentUserData.startYear;
  }
  document.getElementById("piCategory").value = currentUserData.qualificationCategory || "Спеціаліст";
  document.getElementById("piTitle").value = currentUserData.pedagogicalTitle || "";
  document.getElementById("piExperience").textContent = computeExperienceLabel(currentUserData.startYear);
  document.getElementById("piAvatarPreview").src = currentUserData.photoURL || ("https://ui-avatars.com/api/?name=" + encodeURIComponent(currentUserData.displayNameFull || "Викладач"));
...
document.addEventListener('DOMContentLoaded', () => {
  const fileInput = document.getElementById("piAvatarFile");
  if (fileInput) {
    fileInput.addEventListener('change', () => {
      const file = fileInput.files[0];
      if (!file) return;
      if (file.size > 3 * 1024 * 1024) {
        const err = document.getElementById("piError");
        err.textContent = "Файл завеликий. Максимум 3 МБ.";
        err.style.display = "block";
        return;
      }
      pendingAvatarFile = file;
      document.getElementById("piAvatarPreview").src = URL.createObjectURL(file);
    });
  }
});

window.savePersonalInfo = async function () {
  const lastName = document.getElementById("piLastName").value.trim();
  const firstName = document.getElementById("piFirstName").value.trim();
  const middleName = document.getElementById("piMiddleName").value.trim();
  const startYear = parseInt(document.getElementById("piStartYear").value);
  ...
  if (!lastName || !firstName || !startYear || !category) {
    errEl.textContent = "Заповніть прізвище, ім'я, рік початку діяльності та категорію";
  ...
  try {
    const data = {
      ...
      pedagogicalTitle: title || null
    };

    if (pendingAvatarFile) {
      const path = `avatars/${currentTeacherId}/${Date.now()}_${pendingAvatarFile.name}`;
...
// ========== ПРОДОВЖЕННЯ ЗАВАНТАЖЕННЯ ПІСЛЯ АВТЕНТИФІКАЦІЇ ==========
async function continueAfterAuth() {
  ...
  if (document.getElementById("tab-journal").classList.contains("active") || document.getElementById("tab-attendance")?.classList.contains("active")) {
    await populateJournalFilters();
  }
...
window.switchTab = function(tabId) {
  ...
  if (tabId === 'tab-attendance') { syncAttendanceSelects(); renderAttendance(); }
  if (tabId === 'tab-lessons') loadMyLessons();
};

window.openAddLessonForm = async function() {
  currentEditId = null;
  document.getElementById("lessonTopic").value = "";
  document.getElementById("lessonDate").value = "";
  document.getElementById("newSectionGroup").style.display = "none";
...
window.toggleForm = (formId) => {
  ...
  if (f) {
      const isOpening = (f.style.display === "none" || f.style.display === "");
      f.style.display = isOpening ? "block" : "none";
      
      // Відновлюємо збережену групу при відкритті форми додавання студента
      if (isOpening && formId === 'form-add-student' && !currentEditId) {
          const lastStudentGroup = sessionStorage.getItem(STORAGE_KEYS.lastStudentGroup);
          if (lastStudentGroup && document.querySelector(`#studentGroup option[value="${lastStudentGroup}"]`)) {
              document.getElementById("studentGroup").value = lastStudentGroup;
          }
      }
  }
  if (currentEditId && formId !== 'form-add-student') currentEditId = null;
};
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    if (snap.exists() && snap.data().role === "teacher") {
      ...
      const savedYear = currentUserData.currentAcademicYear || localStorage.getItem(ACADEMIC_YEAR_STORAGE_KEY);
      if (savedYear && academicYearsCache.includes(savedYear)) {
        currentAcademicYear = savedYear;
        localStorage.setItem(ACADEMIC_YEAR_STORAGE_KEY, savedYear);
        document.getElementById("yearBadgeValue").textContent = savedYear;
        await continueAfterAuth();
      } else {
        await window.openYearModal(true);
      }
    } else {
      window.location.href = "index.html";
    }
 ...
 * Навантаження = сума годин лекцій та практичних занять з усіх предметів/дисциплін.
 * Предмет вважається "заповненим повністю", якщо вказано загальну кількість годин
 * і розподіл (лекції + практичні + лабораторні + семінарські + самостійна робота)
 * повністю дорівнює цій загальній кількості годин.
 * @param {Array} subjects - масив об'єктів предметів/дисциплін
...
function calculateTotalWorkload(subjects) {
    ...
    subjects.forEach(subject => {
        ...
        const practicals = parseFloat(subject.practicals) || 0;
        const labs = parseFloat(subject.labs) || 0;
        const seminars = parseFloat(subject.seminars) || 0;
        const selfStudy = parseFloat(subject.selfStudy) || 0;
        const totalHours = parseFloat(subject.hours) || 0;

        const distributed = lectures + practicals + labs + seminars + selfStudy;

        // Розподіл годин вважається незаповненим, якщо загальна кількість годин
        // не вказана, або сума складових не збігається з нею
        if (totalHours <= 0 || distributed !== totalHours) {
            isComplete = false;
        }

        totalLoad += (lectures + practicals);
    });

    return { total: totalLoad, isComplete };
}
...
function updateWorkloadDisplay(subjects) {
    ...
    if (!subjects || subjects.length === 0) {
        container.innerHTML = `<span class="warning-text">Немає предметів/дисциплін для розрахунку педагогічного навантаження.</span>`;
        return;
    }
...
// ========== ДОСТУПНІСТЬ: КЛІКАБЕЛЬНІ КАРТКИ СТАТИСТИКИ З КЛАВІАТУРИ ==========

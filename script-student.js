...
async function renderStudentJournal(subjectId) {
  ...
  const moduleValues = computeModuleValues(lessons, id => grades[id]);
  ...
  lessons.forEach(lesson => {
    if (lesson.type === MODULE_TYPE) {
      const mv = moduleValues[lesson.id];
      html += `<td class="module-cell ${mv !== null ? gradeClassForNumber(mv) : ''}">${mv !== null ? mv : '—'}</td>`;
      return;
    }
    const grade = grades[lesson.id] || '';
    let gradeClass = '';
  ...
  html += `${hasModules ? '' : `<td class="stat-cell ${avgClass}">${avg}</td>`}<td class="${nClass}">${nCount}</td>
